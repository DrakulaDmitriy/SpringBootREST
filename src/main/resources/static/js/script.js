...
async function addNewUser() {
    $('#addNewUserButton').click(async () => {
        ...
        let login = addUserForm.find('#loginNewUser').val().trim();
    ...
    fetch('/api/roles').then(function (response) {
        ...
        response.json().then(roleList => {
            roleList.forEach(role => {
                form.find('#newRoles')
                    .append($('<option>').val(role.id).text(role.userRole));
            })
        })
    })
}
...
async function deleteUser(modal, id) {
    ...
    let deleteButton =
        `<button class="btn btn-danger" id="deleteButton" data-dismiss="modal" data-backdrop="false">Delete</button>`
        ...
        let bodyForm = `<form id="deleteUser">
                            <div class="col-md-7 offset-md-3 text-center">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" value="${user.login}" name="login" class="form-control" readonly>
                                </div>`
...
async function editUser(modal, id) {
    ...
    user.then(user => {
        let bodyForm = `<form id="editUser">
                            <div class="col-md-7 offset-md-3 text-center">
                                ...
                                <div class="form-group">
                                    <span class="font-weight-bold">Login</span>
                                    <input type="text" value="${user.login}" name="login" id="login" class="form-control">
                                </div>
                                <div class="form-group">
    ...
    $("#editButton").on('click', async () => {
        ...
        let login = modal.find('#login').val()
